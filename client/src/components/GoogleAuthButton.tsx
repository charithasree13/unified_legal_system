...
export const GoogleAuthButton: React.FC<GoogleAuthButtonProps> = ({
  ...
  const handleButtonClick = () => {
    ...
    // 1. Try Google OAuth 2.0 Token Client Popup with prompt: 'select_account'
      ...
      try {
        const client = window.google.accounts.oauth2.initTokenClient({
          ...
          error_callback: (err: any) => {
            ...
            if (onError) onError('Google Sign-In blocked (Error 400: origin_mismatch). Please add https://www.elitelegaldesk.com to Authorized JavaScript Origins in Google Cloud Console.');
            setLoading(false);
          }
        });
    ...
    // 2. Try GIS One Tap / Standard Prompt
    if (window.google?.accounts?.id) {
      initGoogleId();
    ...
    if (onError) {
      onError('Google Sign-In service is unavailable. Please use password login or register an account.');
    }
    setLoading(false);
  };
