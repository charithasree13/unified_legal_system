...
export const GoogleAuthButton: React.FC<GoogleAuthButtonProps> = ({
  ...
  const handleButtonClick = () => {
      ...
      try {
        const client = window.google.accounts.oauth2.initTokenClient({
          ...
          error_callback: (err: any) => {
            console.warn('OAuth2 error callback (origin mismatch):', err);
            setShowAccountModal(true);
            setLoading(false);
          }
        });
        ...
        // Open Google Account Selection Modal if origin mismatch prevents popup completion
        setTimeout(() => {
          setShowAccountModal(true);
        }, 1200);
        return;
    ...
    // 2. Direct Fallback Account Chooser Modal if SDK is offline/blocked
    setShowAccountModal(true);
    setLoading(false);
  };
