...
export const GoogleAuthButton: React.FC<GoogleAuthButtonProps> = ({
  ...
  const sendAuthPayload = async (credential: string) => {
    setLoading(true);
    if (onStart) onStart();
  ...
  const handleCredentialResponse = (response: any) => {
    ...
    sendAuthPayload(response.credential);
  };
  ...
  const handleTriggerGooglePrompt = () => {
    ...
    if (window.google?.accounts?.id) {
      try {
        window.google.accounts.id.prompt((notification: any) => {
          if (notification.isNotDisplayed() || notification.isSkippedMoment()) {
            // If One-tap overlay isn't displayed or skipped, focus or click GIS container button
            const btn = gisContainerRef.current?.querySelector('[role="button"]') as HTMLElement;
            if (btn) btn.click();
          }
        });
      } catch (e) {
      ...
      if (onError) onError('Google Identity Services is initializing. Please try again in a moment.');
    }
  };
  ...
  return (
    <div className={`w-full flex flex-col items-center justify-center my-1 ${className}`}>
      ...
      <div 
        ref={gisContainerRef} 
        className="w-full flex justify-center overflow-hidden my-1 min-h-[44px]"
      />

      {/* Styled Fallback / Supplementary Action Button */}
      {(!gisLoaded || loading) && (
        <button
          type="button"
